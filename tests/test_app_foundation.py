...
def test_logout_clears_persistent_remember_cookie(client, app):
    ...
    # Login uses remember=True, so the configured persistent token must be issued.
    assert any(
        f"{remember_cookie_name}=" in header
    ...
    # Flask-Login must explicitly expire the configured persistent token on logout.
    remember_headers = [
        header
        for header in logout_response.headers.getlist("Set-Cookie")
        if f"{remember_cookie_name}=" in header
    ]
    assert remember_headers
    assert any(
        f"{remember_cookie_name}=;" in header
        and ("Expires=" in header or "Max-Age=0" in header)
