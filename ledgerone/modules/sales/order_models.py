...
class SalesOrder(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint("organisation_id", "order_number", name="uq_sales_order_org_number"),
    )
    ...
    requested_delivery_date = db.Column(db.Date, nullable=True, index=True)
...
class SalesOrderLine(db.Model):
    ...
    id = db.Column(db.String(36), primary_key=True, default=new_id)
    order_id = db.Column(db.String(36), db.ForeignKey("sales_orders.id", ondelete="CASCADE"), nullable=False, index=True)
